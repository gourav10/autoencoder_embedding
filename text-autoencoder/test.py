...
if __name__ == '__main__':
    ...
    if args.encode:
        fa, fb = args.data.split(",")
        sa, sb = load_sent(fa), load_sent(fb)
        za, zb = encode(sa), encode(sb)
        z = np.concatenate((za, zb), axis=0)
        # np.savetxt(args.output, za)
        a_len = len(za)
        b_len = len(zb)
        labels = [1] *a_len + [0] *b_len

        pca = PCA(n_components=2) # project from 784 to 2 dimensions
        principalComponents = pca.fit_transform(z)
        principal_df = pd.DataFrame(data = principalComponents
                    , columns = [f'principal component {i}' for i in range(2)])
        ...
        silhscore = metrics.silhouette_score(principal_df, kmeans.labels_)
        # print(silhscore)

        plt.scatter(principalComponents[:, 0], principalComponents[:, 1], s= 5, color = ['red' if l == 1 else 'green' for l in labels], cmap='Spectral')
        ...
        # plt.scatter(tsne_results[:, 0], tsne_results[:, 1], color = ['red' if l == 1 else 'green' for l in labels])
        plt.show()
